...
### Agent 主动检索（Agent RAG）

知识库可选用 **Agent 主动检索方案**，从"被动检索"升级为"主动检索"：

- **导入期主动分析**：素材导入后，后台任务自动调用 LLM 为每个文档生成索引摘要（标题 / 摘要 / 关键词 / 典型问题），并指向原文档；索引状态可轮询、可一键重建
- **对话期自主决策**：基于 LangGraph ReAct Agent，模型自主完成意图识别——闲聊直接回答，涉及资料时主动调用 `kb_list_documents`（浏览素材清单）与 `kb_search`（两段式取料：索引摘要层定位文档 → 文档块级精确检索），可多轮换词检索
- **可靠降级**：langgraph 内核 → 内置 function-calling 循环 → 标准 RAG 检索，三级降级保证任何环境可用
- **存储解耦**：方案不绑定向量库，存储按 KB 级配置 > 环境变量 > 组件配置 > 方案默认解析，Agent 索引与文档块向量始终落在同一后端
- **会话记忆**：sqlite checkpoint 按会话持久化 Agent 状态，重启不丢
- **引用溯源**：回答标注来源文档名，对话页实时展示"思考 → 检索 → 命中素材"过程
...
- 融合与重排
- 基于知识库的问答（含真流式输出）
- Agent 主动检索（导入期索引摘要 + LangGraph ReAct 自主检索 + 来源溯源）
