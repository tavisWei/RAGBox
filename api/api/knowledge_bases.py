...
RAG_PLAN_PRESETS = {
    ...
    "agent": {
        "key": "agent",
        "name": "Agent 主动检索方案",
        "summary": "主动 Agent RAG：素材导入后由 LLM 生成索引摘要并指向原文档；对话时 LangGraph ReAct Agent 自主识别意图、浏览素材清单、按需多轮检索取料，再加工回答。",
        "cost_level": "中",
        "quality_level": "智能路由",
        "architecture": "导入期后台任务生成索引摘要（标题/关键词/典型问题）并写入 {kb}__agent_index 索引集合；对话期 LangGraph ReAct Agent 通过 kb_list_documents（浏览素材清单）与 kb_search（两段式：索引摘要层定位文档 → 文档块级取料）自主检索，sqlite checkpoint 持久化会话，三级降级（langgraph → 内置 function-calling 循环 → 标准检索）",
        "recommended_backend": "sqlite",
        "vector_backend": "sqlite-builtin",
        "datastore_note": "默认沿用 SQLite 本地存储，可在存储配置/组件配置/环境变量中切换 pgvector/ES 等后端；索引摘要与块向量共存于同一后端。",
